...
int main()
{
	...
	if(socketpair(AF_LOCAL,SOCK_DGRAM,0,sockets)==-1){
		fprintf(stderr,"Unable to create socket pairs");
		return 1;
	}
	int err;
	...
	while(1)//infinite while loop
	{
		...
		if(FD_ISSET(sockets[1],&readfds)){
			read(sockets[1],receive_buf,10);
			printf("fell through\n");
			continue;
		}
		printf("data received on sockfd");
		...
		{
			//write(1,receive_buf+1,1);
			node_table[atoi(receive_buf+1)][5+mode*4]|=1;
		}
		...
		else if(strncmp(receive_buf,"l",1)==0)
		{
			...
			for(int i=current_index+1;i!=current_index;i=(i+1)%LOG_SIZE)
			{
				...
				//usleep(TRANSMISSION_DELAY);//Time for the receiver to get ready for next frame
			}
...
void heart_scheduler(int sig)//periodic function to call the heart model every 1ms
{
	...
	digitalWrite(ASENSE_PIN,node_table[0][9]);
	digitalWrite(VSENSE_PIN,node_table[nx-1][9]);
	displayAndLogData(0);
	//printf("%ld\n",run_count);
	run_count++;
	...
	while(1)
	{
		...
		while(1){	
			...
			(struct sockaddr *)&their_addr, addr_len)) == -1) {
				...
				ready_to_receive=0;
				char dummy[2];
				write(sockets[0],dummy,sizeof(dummy));
				break;
			}
